...
### MongoDB from Python tutorial => https://realpython.com/introduction-to-mongodb-and-python/
### MongoDB installation => https://docs.mongodb.com/manual/tutorial/install-mongodb-on-os-x/
### Unity tutorial => https://developer.mongodb.com/how-to/sending-requesting-data-mongodb-unity-game/
...
### DB Connection
connect(db="ssexpo", host="localhost", port=27017)
...
while True:
	...
	if len(people) > 0:
		...
		label = "{}: {:.2f}% height={}".format("Person", closest_person.confidence * 100, target_h)
		...
		# Write the height value to MongoDB
		# DB name: ssexpo
		# Collection: data
		# Structure: {"description":String, "value":Int}
		QueriedData.objects(description="User proximity value").update_one(value=target_h)

		# Not necessary
		# people.clear()
	else:
		# If no person is detected, just write 1
		QueriedData.objects(description="User proximity value").update_one(value=-1)
